...
$result = ExecuteCommand "func azure functionapp fetch-app-settings $namePrefix-audio-api" $loggingPrefix "Fetching the API app settings from azure."
...
$result = ExecuteCommand "func settings add ""Host.LocalHttpPort"" ""7073""" $loggingPrefix "Adding the worker run time port setting for '7073'."

Set-Location "$directoryStart/src/ContentReactor.$microserviceName/ContentReactor.$microserviceName.WorkerApi"
$result = ExecuteCommand "func azure functionapp fetch-app-settings $namePrefix-audio-worker" $loggingPrefix "Fetching the worker app settings from azure."
...
$result = ExecuteCommand "func settings add ""Host.LocalHttpPort"" ""7074""" $loggingPrefix "Adding the worker run time port setting for '7074'."
