...
if($IsWindows -and $false) {
    Write-BuildInfo "Installing dotnet core." $loggingPrefix
    ...
    Write-BuildInfo "Installing git." $loggingPrefix
    winget install -e --name Git
    Write-BuildInfo "Installing Visual Studio Code." $loggingPrefix
    winget install -q vscode
    Write-BuildInfo "Installing Azure Powershell Module." $loggingPrefix
    Install-Module -Name Az -AllowClobber -Scope CurrentUser
    Write-BuildInfo "Installing Azure Release Management (ARM) Module." $loggingPrefix
    Install-Module -Name AzureRm -AllowClobber -Scope CurrentUser
    Write-BuildInfo "Installing Asure CLI." $loggingPrefix
    Invoke-WebRequest -Uri https://aka.ms/installazurecliwindows -OutFile .\AzureCLI.msi; Start-Process msiexec.exe -Wait -ArgumentList '/I AzureCLI.msi /quiet'; rm .\AzureCLI.msi
    Write-BuildInfo "Installing Node.js." $loggingPrefix
    winget install openjs.nodejs
    Write-BuildInfo "Installing Azure Functions Core Tools." $loggingPrefix
