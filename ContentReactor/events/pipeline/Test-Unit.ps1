...
if ($Continuous) {
    $testJob = Start-Job -Name "test-continuous" -ScriptBlock {
        ...
        dotnet watch --project ./../tests/ContentReactor.Events.Tests.csproj test `
    ...
    While ($testJob) {
        ...
        If ($Host.UI.RawUI.KeyAvailable -and ($Key = $Host.UI.RawUI.ReadKey("AllowCtrlC,NoEcho,IncludeKeyUp"))) {
            If ([Int]$Key.Character -eq 3) {
                ...
                Write-BuildInfo "Shutting down testing job." $loggingPrefix
    ...
    $testJob = Start-Job -Name "test-continuous" -ScriptBlock {
        ...
        . ./Functions.ps1
        $command = @"
        dotnet test ./../tests/ContentReactor.Events.Tests.csproj ``
